...
//! Struct describing a timevalue as a std::chrono::time_point with a monotonic microsecond-accurate timer
struct Timestamp	{
	...
	Timestamp() : rawTime( std::chrono::time_point_cast<RawTSDuration>(RawTSClockType::now()) )	{}
	
	//!	Creates a new Timestamp with the passed time as a chrono::time_point< chrono::steady_clock, chrono::microseconds> >
	Timestamp(const RawTSTime & inRawTime) : rawTime(inRawTime)	{}
	
	//!	Creates a new Timestamp with the passed time as a chrono::microseconds
	Timestamp(const RawTSDuration & inRawDuration) : rawTime( RawTSTime(inRawDuration) )	{}
	
	//!	Creates a new Timestamp with the passed time in seconds as a double
	Timestamp(const double & inTimeInSeconds) : rawTime( std::chrono::time_point_cast<RawTSDuration>( DoubleTimeTime( DoubleTimeDuration(inTimeInSeconds) ) ) ) {}
